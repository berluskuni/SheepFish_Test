...
from django_drf.celery import app
from django.http import JsonResponse
from printer.models import Check, Printer
        ...
        if Printer.objects.filter(point_id=int(point_id)).exists():
            ...
            for printer in printers:
                if printer.check_type == 'kitchen':
                    ...
                    app.send_task('printer.jobs.tasks.rendered_pdf_kitchen', [data_task])
                else:
                    data_models = {'printer': printer, 'type': 'client', 'status': data.get('status', 'new'),
                                   'order': order_dict}
                    check = Check.objects.create(**data_models)
                    data_task = {'check_id': check.id}
                    app.send_task('printer.jobs.tasks.rendered_pdf_client', [data_task])
